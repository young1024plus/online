      ...
      <ul>
        <li v-for="(item,index) in list" :key="index" @click="goinfo(item._id)">
          ...
          <div class="img">
            ...
            <span class="noimg" v-else>无图</span>
          </div>
        </li>
      </ul>
    </div>
...
export default {
  ...
  mounted() {
    ...
    document.getElementById('content').addEventListener('scroll',function(){
      ...
      // console.log(this.getElementsByTagName("ul")[0].offsetHeight);
      var sctop = this.scrollTop;
      var sheight = this.getElementsByTagName("ul")[0].offsetHeight;
      var boxheight = this.offsetHeight;
      // 2000 - 655  = 1345 

      if(sheight - boxheight == sctop ){
        console.log('bottom')

        that.$axios.post('/news',{
            type: that.nav[that.cur] ,
            page: that.page+1 
        }).then(res=>{
